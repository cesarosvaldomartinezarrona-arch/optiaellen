...
const phases = [
  ...
  { name: 'Control de Calidad', icon: ShieldCheck, color: 'text-orange-600 bg-orange-100 border-orange-200' },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Laboratorio</h1>
          <p className="text-slate-500 text-sm mt-1">Seguimiento de órdenes de taller</p>
        </div>
      ...
      <div className="space-y-5">
        {labOrders.map(order => (
          <div key={order.id} className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 hover:shadow-lg transition-all duration-300">
            <div className="flex items-start justify-between mb-5">
              <div>
                <div className="flex items-center gap-3 mb-1">
                  <span className="text-sm font-bold text-[#7c3aed]">{order.id}</span>
                  <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold ${order.phase === 6 ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-amber-50 text-amber-700 border border-amber-200'}`}>{order.status}</span>
                </div>
                <h3 className="text-base font-bold text-slate-900">{order.patientName}</h3>
              </div>
              {order.phase < 6 && (
                <button onClick={() => handleAdvance(order.id)}
                  className="flex items-center gap-1.5 bg-purple-50 hover:bg-purple-100 text-[#7c3aed] px-4 py-2 rounded-xl text-sm font-bold transition-colors border border-purple-200">
                  Avanzar fase <ChevronRight className="w-4 h-4" />
                </button>
              )}
            </div>
            <p className="text-sm text-slate-500 mb-4">{order.products}</p>
            <div className="flex items-center gap-6 mb-5 text-sm text-slate-500">
              <div className="flex items-center gap-1.5"><User className="w-4 h-4 text-slate-400" />{order.operator}</div>
              <div className="flex items-center gap-1.5"><Calendar className="w-4 h-4 text-slate-400" />Inicio: {order.startDate}</div>
              <div className="flex items-center gap-1.5"><Clock className="w-4 h-4 text-slate-400" />Entrega: {order.estimatedDelivery}</div>
            </div>
            <div className="flex items-center gap-2 bg-slate-50 rounded-xl p-4 border border-slate-100">
              {phases.map((phase, idx) => {
                ...
                return (
                  <div key={phase.name} className="flex-1 flex flex-col items-center">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-1.5 transition-all border-2 ${
                      isCompleted ? 'bg-[#7c3aed] text-white border-[#7c3aed] shadow-md shadow-purple-500/30' :
                      isCurrent ? `${phase.color} ring-4 ring-purple-100` :
                      'bg-white text-slate-300 border-slate-200'
                    }`}>
                      <PhaseIcon className="w-5 h-5" />
                    </div>
                    <p className={`text-center text-[9px] font-bold ${isCurrent ? 'text-[#7c3aed]' : isCompleted ? 'text-slate-700' : 'text-slate-400'}`}>{phase.name}</p>
                  </div>
                );
              })}
            </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl border border-slate-200/80">
            <div className="p-6 border-b border-slate-100"><h2 className="text-lg font-bold text-slate-900">Nueva Orden de Laboratorio</h2></div>
