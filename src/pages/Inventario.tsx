    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg w-full max-w-2xl shadow-2xl border border-slate-200/80 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-end gap-3 p-6 sm:p-8 border-t border-slate-100 bg-slate-50/50 rounded-b-3xl">
          ...
          <button onClick={onSave} className="px-7 py-3 rounded-lg text-sm font-semibold bg-gradient-to-r from-[#7c3aed] to-[#6d28d9] text-white shadow-lg shadow-purple-500/25 flex items-center gap-2 hover:shadow-purple-500/30 transition-all"><Save className="w-4 h-4" /> Guardar</button>
        </div>
      </div>
    </div>
  );
            ...
            {['Todos', 'Monturas', 'Lentes', 'Cristales', 'Accesorios'].map((cat, i) => {
              ...
              return (
                <button key={cat} onClick={() => setFilterCategory(full as any)}
                  className={`px-4 py-2.5 rounded-lg text-xs font-semibold transition-all whitespace-nowrap ${filterCategory === full ? 'bg-[#7c3aed] text-white shadow-md' : 'bg-slate-50 text-slate-500 hover:bg-slate-100 border border-slate-200'}`}>
                  {cat}
                </button>
              );
            })}
