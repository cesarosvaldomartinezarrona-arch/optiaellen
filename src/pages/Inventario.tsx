    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Control de productos y stock</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm">
          ...
          <p className="text-3xl font-extrabold text-[#7c3aed] mt-2">{totalProducts}</p>
        </div>
        <div className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm">
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Valor en Stock</p>
          <p className="text-3xl font-extrabold text-emerald-600 mt-2">${totalValue.toLocaleString()}</p>
        </div>
        <div className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm">
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Stock Bajo</p>
          <div className="flex items-center gap-2 mt-2"><AlertTriangle className="w-6 h-6 text-red-500" /><p className="text-3xl font-extrabold text-red-600">{lowStockCount}</p></div>
        </div>
      </div>

      <div className="bg-white rounded-2xl border border-slate-200/80 p-4 flex flex-wrap gap-4 items-center shadow-sm">
        ...
        <div className="flex gap-2 flex-wrap">
          {['Todos', 'Monturas', 'Lentes de Contacto', 'Cristales', 'Accesorios'].map(cat => (
            <button key={cat} onClick={() => setFilterCategory(cat as any)}
              className={`px-3 py-2 rounded-xl text-xs font-semibold transition-all ${filterCategory === cat ? 'bg-[#7c3aed] text-white shadow-md' : 'bg-slate-50 text-slate-500 hover:bg-slate-100 border border-slate-200'}`}>
              {cat}
            </button>
          ))}
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map(product => (
                <tr key={product.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4"><span className={`inline-flex px-2.5 py-1 rounded-full text-[11px] font-bold ${categoryColors[product.category]}`}>{product.category}</span></td>
