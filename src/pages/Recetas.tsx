...
import { Plus, Eye, Calendar, Stethoscope, X, FileDown, Printer, Check, Glasses, Scissors as Treatment } from 'lucide-react';
...
const emptyEye: EyeData = { sph: '', cyl: '', axis: '', add: '', dp: '', av: '' };
...
function generatePDF(opticsName: string, rx: Prescription) {
  ...
  const drawEyeData = (x: number, title: string, data: EyeData) => {
    ...
    doc.setFillColor(255, 255, 255); doc.setDrawColor(226, 232, 240); doc.roundedRect(x, eyeY + 10, colW, 50, 2, 2, 'FD');
    const fields = [['Esfera', data.sph], ['Cilindro', data.cyl], ['Eje', data.axis], ['Adición', data.add], ['DP', data.dp], ['Agudeza', data.av]];
...
export default function Recetas() {
  ...
  const EyeForm = ({ label, data, onChange, readonly = false }: { label: string; data: EyeData; onChange: (d: EyeData) => void; readonly?: boolean }) => (
    <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
      <h4 className="text-sm font-bold text-[#7c3aed] mb-3">{label}</h4>
      <div className="grid grid-cols-3 gap-3">
        {(['sph', 'cyl', 'axis', 'add', 'dp', 'av'] as const).map(field => (
          <div key={field}>
            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">{field === 'av' ? 'Agudeza Visual' : field}</label>
            <input type="text" value={data[field]} onChange={e => onChange({ ...data, [field]: e.target.value })} readOnly={readonly}
              className="w-full px-3 py-2 bg-white rounded-lg border border-slate-200 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#7c3aed]/20 focus:border-[#7c3aed] mt-1" />
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {filtered.map(rx => (
          <div key={rx.id} className="bg-white rounded-2xl border border-slate-200/80 shadow-sm hover:shadow-lg hover:shadow-slate-200/50 transition-all duration-300 overflow-hidden">
            <div className="p-5">
              <div className="flex items-center justify-between mb-4">
                <span className="text-xs font-bold text-[#7c3aed] bg-purple-50 px-3 py-1 rounded-lg">{rx.id}</span>
                <span className={`px-2.5 py-1 rounded-full text-[11px] font-semibold ${statusColor(rx.status)}`}>{rx.status}</span>
              </div>
              <h3 className="font-bold text-slate-900 text-base mb-3">{rx.patientName}</h3>
              <div className="space-y-2 text-sm">
                <div className="flex items-center gap-2 text-slate-500"><Calendar className="w-4 h-4" />{rx.date}</div>
                <div className="flex items-center gap-2 text-slate-500"><Stethoscope className="w-4 h-4" />{rx.doctor}</div>
              </div>
              <div className="mt-4 grid grid-cols-2 gap-3">
                <div className="bg-slate-50 rounded-xl p-3 text-center border border-slate-100">
                  <p className="text-[10px] font-bold text-slate-400 uppercase">OD</p>
                  <p className="text-lg font-extrabold text-[#7c3aed] mt-1">{rx.rightEye.sph || 'N/A'}</p>
                </div>
                <div className="bg-slate-50 rounded-xl p-3 text-center border border-slate-100">
                  <p className="text-[10px] font-bold text-slate-400 uppercase">OI</p>
                  <p className="text-lg font-extrabold text-[#7c3aed] mt-1">{rx.leftEye.sph || 'N/A'}</p>
                </div>
              </div>
              {rx.grandTotal > 0 && (
                <div className="mt-3 bg-gradient-to-r from-purple-50 to-slate-50 rounded-xl p-3 border border-purple-100">
            ...
            <div className="border-t border-slate-100 p-3 bg-slate-50/50 flex gap-2">
              <button onClick={() => setViewPrescription(rx)} className="flex-1 flex items-center justify-center gap-2 text-[#7c3aed] hover:text-[#5b21b6] text-sm font-semibold py-2 rounded-xl hover:bg-purple-50 transition-all">
                <Eye className="w-4 h-4" /> Ver
              </button>
              <button onClick={() => generatePDF(opticsName, rx)} className="flex-1 flex items-center justify-center gap-2 text-emerald-600 hover:text-emerald-700 text-sm font-semibold py-2 rounded-xl hover:bg-emerald-50 transition-all">
                <FileDown className="w-4 h-4" /> PDF
              </button>
              <button onClick={() => openTicketWindow(buildTicketHTML(opticsName, rx))} className="flex-1 flex items-center justify-center gap-2 text-amber-600 hover:text-amber-700 text-sm font-semibold py-2 rounded-xl hover:bg-amber-50 transition-all">
          ...
          <div className="bg-white rounded-2xl w-full max-w-full sm:max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200/80">
            <div className="flex items-center justify-between p-6 border-b border-slate-100 sticky top-0 bg-white z-10">
              <div>
                ...
                <p className="text-sm text-slate-500">{viewPrescription.patientName} — {viewPrescription.date}</p>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => generatePDF(opticsName, viewPrescription)} className="flex items-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded-xl text-sm font-semibold transition-all shadow-md">
                  <FileDown className="w-4 h-4" /> PDF
                </button>
                <button onClick={() => openTicketWindow(buildTicketHTML(opticsName, viewPrescription))} className="flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white px-4 py-2 rounded-xl text-sm font-semibold transition-all shadow-md">
                  <Printer className="w-4 h-4" /> Ticket
                </button>
                <button onClick={() => setViewPrescription(null)} className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center"><X className="w-4 h-4 text-slate-500" /></button>
              </div>
            </div>
            <div className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-5">
                <EyeForm label="Ojo Derecho (OD)" data={viewPrescription.rightEye} onChange={() => {}} readonly />
                <EyeForm label="Ojo Izquierdo (OI)" data={viewPrescription.leftEye} onChange={() => {}} readonly />
              </div>
              {viewPrescription.selectedLenses && viewPrescription.selectedLenses.length > 0 && (
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                  <h4 className="text-sm font-bold text-slate-900 mb-3 flex items-center gap-2"><Glasses className="w-4 h-4 text-[#7c3aed]" /> Lentes Seleccionados</h4>
                  <div className="space-y-2">
                    {viewPrescription.selectedLenses.map(l => (
                      <div key={l.id} className="flex justify-between items-center bg-white rounded-lg p-3 border border-slate-100">
                        <div>
              ...
              {viewPrescription.treatments && viewPrescription.treatments.length > 0 && (
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                  <h4 className="text-sm font-bold text-slate-900 mb-3 flex items-center gap-2"><Treatment className="w-4 h-4 text-[#7c3aed]" /> Tratamientos</h4>
                  <div className="space-y-2">
                    {viewPrescription.treatments.map(t => (
                      <div key={t.id} className="flex justify-between items-center bg-white rounded-lg p-3 border border-slate-100">
                        <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                  ...
                  <p className="text-sm text-slate-600">{viewPrescription.observations || 'Sin observaciones.'}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-full sm:max-w-5xl max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200/80">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wider">Doctor</label>
              ...
              {/* Eye Data */}
              <div className="grid grid-cols-2 gap-5">
                <EyeForm label="Ojo Derecho (OD)" data={newRx.rightEye} onChange={d => setNewRx({ ...newRx, rightEye: d })} />
                <EyeForm label="Ojo Izquierdo (OI)" data={newRx.leftEye} onChange={d => setNewRx({ ...newRx, leftEye: d })} />
              </div>

              {/* Lenses Selection */}
              <div className="bg-slate-50 rounded-xl p-5 border border-slate-200">
                <h4 className="text-sm font-bold text-slate-900 mb-3 flex items-center gap-2"><Glasses className="w-4 h-4 text-[#7c3aed]" /> Seleccionar Lentes</h4>
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                  {availableLenses.map(lens => {
                    const isSelected = newRx.selectedLenses.some(l => l.id === lens.id);
                    return (
                      <button key={lens.id} onClick={() => toggleLens(lens)}
                        className={`text-left p-3 rounded-xl border-2 transition-all ${isSelected ? 'border-[#7c3aed] bg-purple-50 shadow-md' : 'border-slate-200 bg-white hover:border-slate-300'}`}>
                        ...
                        <p className="text-sm font-extrabold text-[#7c3aed] mt-1">${lens.price.toLocaleString()} <span className="text-[10px] font-normal text-slate-400">x{lens.quantity}</span></p>
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="bg-slate-50 rounded-xl p-5 border border-slate-200">
                <h4 className="text-sm font-bold text-slate-900 mb-3 flex items-center gap-2"><Treatment className="w-4 h-4 text-[#7c3aed]" /> Seleccionar Tratamientos</h4>
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                  {availableTreatments.map(treatment => {
                    ...
                    return (
                      <button key={treatment.id} onClick={() => toggleTreatment(treatment)}
                        ...
                        <p className="text-sm font-extrabold text-[#7c3aed] mt-1">${treatment.price.toLocaleString()}</p>
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <textarea value={newRx.observations} onChange={e => setNewRx({ ...newRx, observations: e.target.value })} rows={3}
                    className="w-full px-4 py-2.5 bg-slate-50 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#7c3aed]/20 focus:border-[#7c3aed] resize-none" />
                </div>
              </div>
            </div>
            <div className="flex justify-end gap-3 p-6 border-t border-slate-100 sticky bottom-0 bg-white">
              ...
              <button onClick={handleAdd} className="px-5 py-2.5 rounded-xl text-sm font-semibold bg-gradient-to-r from-[#7c3aed] to-[#6d28d9] text-white shadow-lg shadow-purple-500/25 hover:shadow-purple-500/40 transition-all">Guardar Examen</button>
            </div>
          </div>
        </div>
      )}
