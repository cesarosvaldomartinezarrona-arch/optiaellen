...
export default function Recetas() {
  ...
  const EyeForm = ({ label, data, onChange, readonly = false }: { label: string; data: EyeData; onChange: (d: EyeData) => void; readonly?: boolean }) => {
    ...
    return (
      <div className="bg-[#fafaf8] rounded-2xl border border-slate-200/70 p-5 sm:p-6 shadow-sm">
        <div className="flex items-center gap-3 mb-5 pb-4 border-b border-slate-200/60">
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center shadow-sm ${isOD ? 'bg-gradient-to-br from-[#7c3aed] to-[#6d28d9] text-white' : 'bg-gradient-to-br from-[#2563eb] to-[#1d4ed8] text-white'}`}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={data.sph} onChange={e => onChange({ ...data, sph: e.target.value })} readOnly={readonly}
              placeholder="+0.00"
              className={`w-full px-4 py-3.5 rounded-xl border text-sm font-semibold placeholder:text-slate-300 focus:outline-none focus:ring-2 transition-all ${readonly ? 'bg-slate-50 border-slate-200 text-slate-700' : 'bg-white border-slate-200 text-slate-800 focus:ring-[#7c3aed]/20 focus:border-[#7c3aed] shadow-sm'}`} />
          </div>
          <div>
        ...
        <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-slate-200/40">
          <div>
            ...
            <input type="text" value={data.dp} onChange={e => onChange({ ...data, dp: e.target.value })} readOnly={readonly}
              placeholder="32"
              className={`w-full px-3.5 py-2.5 rounded-xl border text-sm font-medium placeholder:text-slate-300 focus:outline-none focus:ring-2 transition-all ${readonly ? 'bg-slate-50 border-slate-200 text-slate-700' : 'bg-white border-slate-200 text-slate-700 focus:ring-[#7c3aed]/20 focus:border-[#7c3aed]'}`} />
          </div>
          <div>
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <button onClick={() => setShowModal(true)} className="flex items-center justify-center gap-2 bg-gradient-to-r from-[#7c3aed] to-[#6d28d9] hover:from-[#6d28d9] hover:to-[#5b21b6] text-white px-5 py-3 rounded-2xl text-sm font-bold transition-all shadow-lg shadow-purple-500/25 hover:shadow-purple-500/40 hover:-translate-y-0.5">
          <Plus className="w-4 h-4" /> Nuevo Examen
        </button>
      </div>

      <div className="flex gap-1 bg-white rounded-2xl p-1.5 border border-slate-200/60 w-fit shadow-sm">
        {(['todas', 'vigentes', 'pasadas'] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={`px-5 py-2.5 rounded-xl text-sm font-semibold transition-all capitalize ${activeTab === tab ? 'bg-[#7c3aed] text-white shadow-md shadow-purple-500/25' : 'text-slate-500 hover:bg-slate-50'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {filtered.map(rx => (
          <div key={rx.id} className="bg-white rounded-2xl border border-slate-200/60 shadow-sm hover:shadow-lg hover:shadow-slate-200/50 hover:-translate-y-1 transition-all duration-300 overflow-hidden group">
            <div className="p-5">
              <div className="flex items-center justify-between mb-4">
                <span className="text-xs font-bold text-[#7c3aed] bg-purple-50 px-3 py-1.5 rounded-xl border border-purple-100">{rx.id}</span>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <div className="bg-gradient-to-br from-purple-50 to-indigo-50/50 rounded-2xl p-3.5 text-center border border-purple-100/50">
                ...
                <div className="bg-gradient-to-br from-blue-50 to-sky-50/50 rounded-2xl p-3.5 text-center border border-blue-100/50">
            ...
            <div className="border-t border-slate-100 p-3 bg-slate-50/30 flex gap-2">
              <button onClick={() => setViewPrescription(rx)} className="flex-1 flex items-center justify-center gap-2 text-[#7c3aed] hover:text-white hover:bg-[#7c3aed] text-sm font-semibold py-2.5 rounded-xl hover:shadow-md transition-all">
                <Eye className="w-4 h-4" /> Ver
              </button>
              <button onClick={() => generatePDF(opticsName, rx)} className="flex-1 flex items-center justify-center gap-2 text-emerald-600 hover:text-white hover:bg-emerald-500 text-sm font-semibold py-2.5 rounded-xl hover:shadow-md transition-all">
                <FileDown className="w-4 h-4" /> PDF
              </button>
              <button onClick={() => openTicketWindow(buildTicketHTML(opticsName, rx))} className="flex-1 flex items-center justify-center gap-2 text-amber-600 hover:text-white hover:bg-amber-500 text-sm font-semibold py-2.5 rounded-xl hover:shadow-md transition-all">
      ...
      {viewPrescription && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#f8f9fb] rounded-3xl w-full max-w-5xl max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200/60">
            <div className="flex items-center justify-between p-6 border-b border-slate-200/60 sticky top-0 bg-white z-10 rounded-t-3xl">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => generatePDF(opticsName, viewPrescription)} className="hidden sm:flex items-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-md">
                  <FileDown className="w-4 h-4" /> PDF
                </button>
                <button onClick={() => openTicketWindow(buildTicketHTML(opticsName, viewPrescription))} className="hidden sm:flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-md">
                  <Printer className="w-4 h-4" /> Ticket
                </button>
                <button onClick={() => setViewPrescription(null)} className="w-9 h-9 rounded-xl bg-slate-100 hover:bg-slate-200 flex items-center justify-center transition-colors"><X className="w-4 h-4 text-slate-500" /></button>
              </div>
            </div>
            <div className="p-6 space-y-6">
                ...
                <div className="bg-white rounded-2xl p-5 border border-slate-200/60 shadow-sm">
                  ...
                  <div className="space-y-2">
                    {viewPrescription.selectedLenses.map(l => (
                      <div key={l.id} className="flex justify-between items-center bg-slate-50 rounded-xl p-3.5 border border-slate-100">
                        <div>
                ...
                <div className="bg-white rounded-2xl p-5 border border-slate-200/60 shadow-sm">
                  ...
                  <div className="space-y-2">
                    {viewPrescription.treatments.map(t => (
                      <div key={t.id} className="flex justify-between items-center bg-slate-50 rounded-xl p-3.5 border border-slate-100">
                        <div>
              ...
              {viewPrescription.grandTotal > 0 && (
                <div className="bg-gradient-to-r from-[#7c3aed] to-[#6d28d9] rounded-2xl p-5 text-white shadow-lg shadow-purple-500/20">
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#f8f9fb] rounded-3xl w-full max-w-5xl max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200/60">
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={newRx.patientId} onChange={e => setNewRx({ ...newRx, patientId: e.target.value })}
                      className="w-full px-4 py-3.5 bg-slate-50 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#7c3aed]/20 focus:border-[#7c3aed] font-medium">
              ...
              {/* Lenses Selection */}
              <div className="bg-white rounded-2xl p-5 sm:p-6 border border-slate-200/60 shadow-sm">
                  ...
                  {availableLenses.map(lens => {
                    ...
                    return (
                      <button key={lens.id} onClick={() => toggleLens(lens)}
                        className={`text-left p-3.5 rounded-2xl border-2 transition-all ${isSelected ? 'border-[#7c3aed] bg-purple-50 shadow-md' : 'border-slate-200 bg-white hover:border-slate-300 hover:shadow-sm'}`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-white rounded-2xl p-5 border border-slate-200/60 shadow-sm">
                  ...
                  <textarea value={newRx.recommendations} onChange={e => setNewRx({ ...newRx, recommendations: e.target.value })} rows={3} placeholder="Notas clínicas y recomendaciones..."
                    className="w-full px-4 py-3 bg-slate-50 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#7c3aed]/20 focus:border-[#7c3aed] resize-none placeholder:text-slate-300" />
                </div>
                <div className="bg-white rounded-2xl p-5 border border-slate-200/60 shadow-sm">
            ...
            <div className="flex justify-end gap-3 p-6 border-t border-slate-200/60 sticky bottom-0 bg-white rounded-b-3xl">
              <button onClick={() => setShowModal(false)} className="px-6 py-3 rounded-xl text-sm font-semibold text-slate-500 hover:bg-slate-100 transition-colors">Cancelar</button>
              <button onClick={handleAdd} className="px-8 py-3 rounded-xl text-sm font-bold bg-gradient-to-r from-[#7c3aed] to-[#6d28d9] text-white shadow-lg shadow-purple-500/25 hover:shadow-purple-500/40 hover:-translate-y-0.5 transition-all">Guardar Examen</button>
            </div>
          </div>
        </div>
      )}
