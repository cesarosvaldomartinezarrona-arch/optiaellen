  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-4">
        {summaryStats.map(s => (
          <div key={s.label} className={`${s.bg} rounded-2xl border border-slate-200/80 p-3 sm:p-5 text-center shadow-sm`}>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {(['todos', 'Preparando', 'Listo para entregar'] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-semibold transition-all whitespace-nowrap ${filter === f ? 'bg-gradient-to-r from-[#7c3aed] to-[#6d28d9] text-white shadow-lg shadow-purple-500/25' : 'bg-white border border-slate-200 text-slate-500 hover:bg-slate-50'}`}>
            {f === 'todos' ? 'Todos' : f}
          </button>
        ))}
      </div>
                ...
                <div className="border-t border-slate-200/50 p-3 bg-white/50">
                  {order.status === 'Listo para entregar' ? (
                    <button onClick={() => handleDeliver(order.id)}
                      className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-bold shadow-md shadow-emerald-500/25 transition-all">
                      <CheckCircle2 className="w-4 h-4" /> Entregar
                    </button>
                  ) : (
                    <button disabled className="w-full flex items-center justify-center gap-2 bg-slate-100 text-slate-400 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-medium cursor-not-allowed">
