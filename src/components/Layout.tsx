        ...
        {items.map(item => {
          ...
          return (
            <NavLink
              key={item.to}
              to={item.to}
              className={`relative flex flex-col items-center justify-center gap-1 px-3 py-2 rounded-2xl transition-all min-w-[64px] ${active ? 'text-white' : 'text-white/40'}`}
            >
          ...
          <header className="h-[64px] bg-white border-b border-slate-200/60 flex items-center justify-between px-4 sm:px-6 lg:px-8 sticky top-0 z-30">
            <div className="flex items-center gap-4 flex-1 min-w-0">
              <button
                ...
                className="lg:hidden relative w-10 h-10 rounded-xl bg-[#0f0a1f] hover:bg-[#1a1033] flex items-center justify-center transition-colors"
              >
              ...
              <div className="hidden lg:flex items-center flex-1 max-w-md ml-8">
                <div className="relative w-full group">
                  ...
                  <input
                    ...
                    placeholder="Buscar..."
                    className="w-full pl-10 pr-4 py-2.5 bg-[#f1f0ff] group-hover:bg-[#ece9ff] rounded-xl border border-transparent text-sm placeholder:text-slate-400 focus:outline-none focus:bg-white focus:border-[#7c3aed]/20 focus:ring-4 focus:ring-[#7c3aed]/10 transition-all"
                  />
                </div>
              </div>
            </div>
            <div className="flex items-center gap-2 sm:gap-3">
              <button className="lg:hidden w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center hover:bg-slate-50 transition-colors">
                <Search className="w-4 h-4 text-slate-600" />
              </button>
              <button className="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center hover:bg-slate-50 transition-colors relative group">
                <Bell className="w-4 h-4 text-slate-600 group-hover:text-[#7c3aed] transition-colors" />
                <span className="absolute top-2 right-2.5 w-2 h-2 bg-red-500 rounded-full border-2 border-white" />
              </button>
              <button className="hidden sm:flex w-10 h-10 rounded-xl bg-white border border-slate-200 items-center justify-center hover:bg-slate-50 transition-colors">
                <Maximize2 className="w-4 h-4 text-slate-600" />
              </button>
              <div className="hidden sm:flex items-center gap-3 pl-3 ml-1 border-l border-slate-200">
                <div className="relative">
                  <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#7c3aed] to-[#6d28d9] flex items-center justify-center shadow ring-2 ring-purple-100">
